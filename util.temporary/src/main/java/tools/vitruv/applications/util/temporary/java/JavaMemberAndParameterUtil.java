...
/**
 * Utility class for creating and manipulating JaMoPP objects for Java members (fields, methods,
 * constructors) and parameters.
 */
@Utility
public final class JavaMemberAndParameterUtil {
  private static final Logger logger =
      Logger.getLogger(JavaMemberAndParameterUtil.class.getSimpleName());
  ...
  /**
   * Creates a simple public Java method with the given name, no return type (void), and no
   * parameters.
   *
   ...
   * @return A new {@link ClassMethod} instance.
   */
  public static ClassMethod createJavaClassMethod(
      final String name,
  ...
  /**
   * Creates a Java interface method with the specified properties. Interface methods are implicitly
   * public.
   *
   * @param name The name of the method.
   * @param returnType The return type of the method. Can be null for void.
   ...
   */
  public static InterfaceMethod createJavaInterfaceMethod(
      final String name, final TypeReference returnType, final List<Parameter> params) {
  ...
  /**
   * A value to initialize a field of an interface with, chosen from the type the model gives it.
   *
   * @param typeName the name of the type the model gives the property, may be null
   * @param manyValued whether the property holds more than one value, in which case the derived
   *     field is a collection and takes {@code null} whatever its element type is
   * @return a fresh literal, never null
   */
  public static Expression createDefaultInitialValue(
      final String typeName, final boolean manyValued) {
    if (manyValued || typeName == null) {
      return LiteralsFactory.eINSTANCE.createNullLiteral();
    }
    switch (typeName) {
      case JavaStandardType.BOOLEAN:
      case "Boolean":
        BooleanLiteral falseLiteral = LiteralsFactory.eINSTANCE.createBooleanLiteral();
        falseLiteral.setValue(false);
        return falseLiteral;
      case JavaStandardType.DOUBLE:
      case "Real":
        DecimalDoubleLiteral zeroDouble = LiteralsFactory.eINSTANCE.createDecimalDoubleLiteral();
        zeroDouble.setDecimalValue(0d);
        return zeroDouble;
      case JavaStandardType.FLOAT:
        DecimalFloatLiteral zeroFloat = LiteralsFactory.eINSTANCE.createDecimalFloatLiteral();
        zeroFloat.setDecimalValue(0f);
        return zeroFloat;
      case JavaStandardType.BYTE:
      case JavaStandardType.CHAR:
      case JavaStandardType.SHORT:
      case JavaStandardType.INT:
      case JavaStandardType.LONG:
      case "Integer":
      case "UnlimitedNatural":
        DecimalIntegerLiteral zeroInteger = LiteralsFactory.eINSTANCE.createDecimalIntegerLiteral();
        zeroInteger.setDecimalValue(java.math.BigInteger.ZERO);
        return zeroInteger;
      default:
        return LiteralsFactory.eINSTANCE.createNullLiteral();
    }
   ...
   */
  public static Field createJavaAttribute(
      final String name,
   ...
   */
  public static List<EnumConstant> createJavaEnumConstantsFromList(
   ...
   */
  public static Constructor addJavaConstructorToClass(
      final Class jClass, final JavaVisibility visibility, final List<Parameter> params) {
   ...
   * @return The newly created and added {@link Constructor}.
   */
  public static Constructor createJavaConstructorAndAddToClass(
      final Class jClass, final JavaVisibility visibility) {
    Constructor constructor = MembersFactory.eINSTANCE.createConstructor();
  ...
  /**
   * Gets the first existing constructor in a class, or creates a new one if none exists. The new
   * constructor will be public by default.
   *
  ...
  /**
   * Creates a getter method for the given field with the specified visibility. The method body will
   * contain "return this.fieldName;".
   *
   * @param jAttribute The field for which to create a getter.
   * @param visibility The desired visibility of the getter.
   * @return A new {@link ClassMethod} representing the getter.
   */
  public static ClassMethod createJavaGetterForAttribute(
      final Field jAttribute, final JavaVisibility visibility) {
    String getterName = buildGetterName(jAttribute.getName());
    TypeReference returnType = EcoreUtil.copy(jAttribute.getTypeReference());
    ClassMethod getterMethod =
        createJavaClassMethod(getterName, returnType, visibility, false, false, null);
  ...
  /**
   * Creates a setter method for the given field with the specified visibility. The method body will
   * contain "this.fieldName = fieldName;".
   *
   * @param jAttribute The field for which to create a setter.
   * @param visibility The desired visibility of the setter.
   * @return A new {@link ClassMethod} representing the setter.
   */
  public static ClassMethod createJavaSetterForAttribute(
      final Field jAttribute, final JavaVisibility visibility) {
    String paramName = toFirstLower(jAttribute.getName());
    OrdinaryParameter param =
        createJavaParameter(paramName, EcoreUtil.copy(jAttribute.getTypeReference()));

    String setterName = buildSetterName(jAttribute.getName());
    ClassMethod setterMethod =
        createJavaClassMethod(
            setterName, null, visibility, false, false, Collections.singletonList(param));

    IdentifierReference paramReference = JavaStatementUtil.createIdentifierReference(param);
    AssignmentExpression attributeAssignment =
    ...
    setterMethod
        .getStatements()
        .add(JavaStatementUtil.wrapExpressionInExpressionStatement(attributeAssignment));
    return setterMethod;
  }

  /**
   * Configures an existing {@link ClassMethod} object to be a setter for the given field. This
   * modifies the passed method object.
   *
  ...
  /**
   * Configures an existing {@link ClassMethod} object to be a getter for the given field. This
   * modifies the passed method object.
   *
  ...
  /**
   * Adds a list of parameters to a parametrizable element (e.g., a method or constructor) if the
   ...
   */
  public static boolean addParametersIfNotNull(
      final Parametrizable parametrizable, final List<Parameter> params) {
  ...
  public static List<ClassMethod> getJavaGettersOfAttribute(final Field jAttribute) {
    ...
    return getJavaGettersOfAttribute(
        jAttribute.getContainingConcreteClassifier(), jAttribute.getName());
  }
  ...
  public static List<ClassMethod> getJavaSettersOfAttribute(final Field jAttribute) {
    ...
    return getJavaSettersOfAttribute(
        jAttribute.getContainingConcreteClassifier(), jAttribute.getName());
  }
   ...
   */
  public static List<ClassMethod> getJavaClassMethodsWithName(
      final ConcreteClassifier jClass, final String name) {
    if (jClass == null) {
      return Collections.emptyList();
    }
   ...
   * @return A list of matching setter methods.
   */
  public static List<ClassMethod> getJavaSettersOfAttribute(
      final ConcreteClassifier jClass, final String jAttributeName) {
   ...
   * @return A list of matching getter methods.
   */
  public static List<ClassMethod> getJavaGettersOfAttribute(
      final ConcreteClassifier jClass, final String jAttributeName) {
    return getJavaClassMethodsWithName(jClass, buildGetterName(jAttributeName));
  }

  /**
   * Renames a setter method to match a new attribute name. This updates the method name, its
   * parameter name, and the field reference within the method body.
   *
   ...
   */
  public static void renameSetter(
      final ClassMethod setter, final Field jAttribute, final String oldName) {
    ...
    for (ExpressionStatement expStatement :
        Iterables.filter(setter.getStatements(), ExpressionStatement.class)) {
      IdentifierReference selfReference =
          JavaStatementUtil.getAttributeSelfReferenceInExpressionStatement(expStatement, oldName);
  ...
  /**
   * Updates the type of a setter's parameter to match a change in the corresponding attribute's
   * type. It also updates the assignment value in the method body.
   *
    ...
    if (param != null) {
      ...
      ExpressionStatement expStatement =
          IterableExtensions.head(
              Iterables.filter(setter.getStatements(), ExpressionStatement.class));
      boolean expressionHasAttributeSelfReference =
          JavaStatementUtil.expressionHasAttributeSelfReference(expStatement, jAttribute);
      if (expressionHasAttributeSelfReference) {
        Expression expression = expStatement.getExpression();
        ((AssignmentExpression) expression)
            .setValue(JavaStatementUtil.createIdentifierReference(param));
      }
    }
  }

  /**
   * Renames a getter method to match a new attribute name. This updates the method name and the
   * field reference in the return statement.
   *
  ...
  public static void renameGetterOfAttribute(ClassMethod getter, Field jAttribute) {
    ...
    Return returnStatement =
        IterableExtensions.head(Iterables.filter(getter.getStatements(), Return.class));
  ...
  /**
   * Updates the return type of a getter method to match a change in the corresponding attribute's
   * type.
   *
  ...
  public static boolean hasSameSignature(final Method method1, final Method method2) {
    ...
    if (JavaTypeUtil.hasSameTargetReference(
        method1.getTypeReference(), method2.getTypeReference())) {
      return false;
    }
    ...
    for (int i = 0; i < params1.size(); i++) {
      if (JavaTypeUtil.hasSameTargetReference(
          params1.get(i).getTypeReference(), params2.get(i).getTypeReference())) {
        return false;
      }
    }
    return true;
  ...
  /**
   * Initializes a class method based on the signature of another method (e.g., an implemented
   * interface method).
   *
   ...
   */
  public static boolean initializeClassMethod(
      final ClassMethod classMethod, final Method implementedMethod, final boolean ensurePublic) {
  ...
  public static boolean initializeClassMethod(
    ...
    if (ensurePublic) {
      boolean alreadyPublic =
          classMethod.getAnnotationsAndModifiers().stream().anyMatch(Public.class::isInstance);
      if (!alreadyPublic) {
  ...
  /**
   * Finds a method in a class that matches the name and number of type parameters of a given
   * method. Note: This is not a full signature match.
   *
   ...
   */
  public static ClassMethod findMethodInClass(
      final ConcreteClassifier concreteClassifier, final ClassMethod method) {
  ...
  /**
   * Assigns a rank to a member for sorting purposes. The order is: Fields (1), Constructors (2),
   * Methods (3), Others (4).
   *

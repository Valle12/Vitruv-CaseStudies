...
public final class JavaModifierUtil {
  ...
  private static final List<Class<? extends Modifier>> JAVA_VISIBILITY_MODIFIER_TYPES =
      Collections.unmodifiableList(
          CollectionLiterals.newArrayList(Public.class, Protected.class, Private.class));
  ...
  public static Iterable<Modifier> getVisibilityModifiers(
      final Iterable<? extends AnnotationInstanceOrModifier> modifiers) {
    return IterableExtensions.filter(
        Iterables.filter(modifiers, Modifier.class), JavaModifierUtil::isVisibilityModifier);
  }

  public static List<String> getModifierNames(
      final Iterable<? extends AnnotationInstanceOrModifier> modifiers) {
    return IterableExtensions.toList(
        IterableExtensions.map(modifiers, it -> it.getClass().getName()));
  }
  ...
  public static void setJavaVisibilityModifier(
      final AnnotableAndModifiable modifiable, final JavaVisibility visibility) {
  ...
  public static void setJavaModifier(
      final AnnotableAndModifiable jModifiable, final Modifier modifier, final boolean add) {
    if (add) {
      if (!jModifiable.hasModifier(modifier.getClass())) {
        jModifiable.addModifier(modifier);
      } else {
        logger.warn(
            "The Java AnnotableAndModifiable "
                + jModifiable.getClass()
  ...
  public static void addModifierIfNotNull(
      final AnnotableAndModifiable jModifiable, final Modifier mod) {
    if (mod != null) {
  ...
  public static void setStatic(final AnnotableAndModifiable modifiable, final boolean toAdd) {
    if (toAdd && modifiable instanceof InterfaceMethod) {
      return;
    }
  ...
  public static JavaVisibility getJavaVisibilityConstantFromUmlVisibilityKind(
      final VisibilityKind uVisibility) {
    if (uVisibility == null) {
  ...
  public static VisibilityKind getUmlVisibilityKindFromJavaVisibilityConstant(
      final JavaVisibility jVisibility) {
    if (jVisibility == null) {
  ...
  public static VisibilityKind getUmlVisibilityKindFromJavaElement(
      final AnnotableAndModifiable javaElement) {
  ...
  public static void setJavaVisibility(
      final AnnotableAndModifiable jModifiable, final VisibilityKind uVisibility) {
    setJavaVisibilityModifier(jModifiable, visibilityOf(jModifiable, uVisibility));
  }

  private static JavaVisibility visibilityOf(
      final AnnotableAndModifiable jModifiable, final VisibilityKind uVisibility) {
    if (jModifiable.eContainer() instanceof Interface) {
      return JavaVisibility.PUBLIC;
    }
    return getJavaVisibilityConstantFromUmlVisibilityKind(uVisibility);
  }
